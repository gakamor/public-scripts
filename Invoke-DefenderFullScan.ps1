...
Write-Output "Full scan started..."
...
$detections = Get-MpThreatDetection |
    Where-Object { $_.InitialDetectionTime -gt $status.FullScanStartTime } |
    ForEach-Object {
...
if ($detections) {
    Write-Output "Malware detected since $($status.FullScanStartTime):"
    $detections
}
else {
    Write-Output "No threats detected since $($status.FullScanStartTime)"
}
